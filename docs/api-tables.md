...
| `autoPageCharWeight`   | `0`     | **deprecated vNEXT** char weight value (adjusts letter spacing) | -1.0 to 1.0. Ex: `{autoPageCharWeight:0.5}` |
| `autoPageLineWeight`   | `0`     | **deprecated vNEXT** line weight value (adjusts line height) | -1.0 to 1.0. Ex: `{autoPageLineWeight:0.5}` |
...
- `autoPageCharWeight`: **deprecated vNEXT.** Adjusts the calculated width of characters. If too much empty space is left on each line,
    then increase char weight value. Conversely, if the table rows are overflowing, then reduce the char weight value.
- `autoPageLineWeight`: **deprecated vNEXT.** Adjusts the calculated height of lines. If too much empty space is left under each table,
