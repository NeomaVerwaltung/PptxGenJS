/**
 * Contracts for `@neo-ma/pptxgenjs-std/charts`.
 *
 * A waterfall is a construction, not a chart type: the riser must stay invisible and each bar must
 * span from the running total to running + delta. Both only hold in the generated package, so these
 * assert on parsed chart XML rather than on the options that went in.
 */
import { test } from 'node:test'
import assert from 'node:assert/strict'
import pptxgen from '../../core/src/pptxgen'
import { assertPptxPackageContracts } from '../../core/test/pptx-contracts'
...
test('waterfall: builds a stacked bar chart with a transparent riser', async () => {
	...
	// Riser: starts each bar at the lower end of its span and must never be visible.
	// Running totals are 100, 145, 115 -> risers 0, 100, 115, and the total bar starts at 0.
	assert.deepEqual(found[0].values, [0, 100, 115, 0])
	assert.ok(found[0].noFill, 'the riser series must be transparent')

	// Visible bars carry the absolute delta, split by sign so each side keeps its own color.
...
test('waterfall: a negative running total still spans from zero', async () => {
	...
	const found = series(await chartXml(pptx))
	// Running totals 10, -15: the second bar hangs from 10 down to -15, so its riser stops at -15.
...
test('waterfall: chart options pass through, but the construction is not overridable', async () => {
	...
	waterfall(pptx.addSlide(), { labels: ['A'], values: [1] }, {
		...
		showLegend: true,
		legendPos: 'b',
		// A caller who sets these would silently break the riser, so the helper wins.
		barDir: 'bar',
